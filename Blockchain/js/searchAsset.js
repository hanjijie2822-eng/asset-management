                ...
                AssetTrackerContract.methods
                    ...
                    .call((error, statusResponse) => {
                        ...
                        if (pendingRequests === 0) {
                            // All data collected, render in order
                            for (let j = 1; j <= statusCount; j++) {
                                let pt = historyPoints[j];
                                if (!pt) continue;
                                let badgeColor = j === 1 ? 'success' : (j === statusCount ? 'danger' : 'info');
                                let stepLabel = j === 1 ? 'Start' : (j === statusCount ? 'Current' : 'Transit');
                                let event =
                                    '<div class="card mb-2 p-2" style="border-left: 4px solid ' +
                                    (j === 1 ? '#28a745' : (j === statusCount ? '#dc3545' : '#17a2b8')) + ';">' +
                                    '<small class="text-muted">' + pt.date.toLocaleString() + '</small><br>' +
                                    '<span class="badge badge-' + badgeColor + '">' + stepLabel + ' #' + j + '</span> ' +
                                    '<strong> ' + pt.owner + '</strong><br>' +
                                    'Status: ' + pt.status + '<br>' +
                                    '<small>Loc: ' + pt.lat.toFixed(4) + ', ' + pt.lng.toFixed(4) + '</small>' +
                                    '</div>';
