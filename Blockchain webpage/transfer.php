    ...
    <script>
      ...
      async function geocodeAndTransfer() {
        ...
        if (!address) {
          ...
          geoResult.textContent = "请输入地址";
          return;
        }

        geoResult.style.color = "#555";
        geoResult.textContent = "正在解析地址...";
        ...
        try {
          ...
          if (!data || data.length === 0) {
            geoResult.style.color = "red";
            geoResult.textContent = "未找到该地址，请重新输入";
            return;
          }
          ...
          geoResult.textContent = `✓ 已解析：${display_name}（经度 ${parseFloat(lon).toFixed(5)}，纬度 ${parseFloat(lat).toFixed(5)}）`;

          // 调用原有的 transferAsset 函数
          transferAsset();
        } catch (err) {
          geoResult.style.color = "red";
          geoResult.textContent = "地址解析失败，请检查网络连接";
        }
      }
    </script>
</body>
